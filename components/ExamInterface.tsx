...
export const ExamInterface: React.FC<ExamInterfaceProps> = ({ user, exam, onComplete, appName, themeColor, settings }) => {
  ...
  */
  ...
  if (exam.formUrl && exam.formUrl.trim() !== '') {
    return (
        <div className="h-screen bg-gray-100 flex flex-col font-sans">
            <div className="bg-white p-4 shadow-sm flex items-center justify-between z-10 border-b">
               <div className="flex items-center gap-3 w-1/3">
                   <div className="w-10 h-10 rounded-full flex items-center justify-center text-white font-bold text-lg pointer-events-none" style={{ backgroundColor: themeColor }}>
                      {appName.charAt(0)}
                   </div>
                   <div>
                       <h1 className="font-bold text-gray-800 pointer-events-none truncate">{exam.title}</h1>
                       <p className="text-xs text-gray-500 pointer-events-none truncate">{user.name}</p>
                   </div>
               </div>
               <div className="text-center w-1/3 text-sm font-bold text-gray-500">
                   Waktu: {formatTime(timeLeft)}
               </div>
               <div className="w-1/3 flex justify-end">
                   <button 
                       onClick={() => {
                          if (confirm("Apakah Anda yakin sudah selesai mengerjakan soal di form ini?")) {
                              handleFinalSubmit();
                          }
                       }}
            ...
            {/* Freeze Screen for Anti-Cheat */}
            {isFrozen && (
               <div className="fixed inset-0 z-[100] bg-red-600 flex items-center justify-center p-4 animate-in fade-in duration-300">
                  <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-8 text-center">
                      ...
                      <h2 className="text-2xl font-black text-gray-800 mb-2">UJIAN DIBLOKIR SEMENTARA</h2>
                      <p className="text-gray-600 mb-6 font-medium">Anda terdeteksi melakukan pelanggaran ujian (membuka tab/aplikasi lain).</p>
                      
                      <div className="bg-red-50 p-6 rounded-lg text-red-800 font-bold text-lg shadow-inner mb-6">
                          Tunggu <span className="text-2xl min-w-[2rem] inline-block">{freezeTimeLeft}</span> detik
                      </div>
                  </div>
               </div>
            )}
            ...
            <div className="p-1 bg-white text-[10px] text-center text-gray-400 border-t">
                Harap menyelesaikan dan submit (Kirim) pada form di atas terlebih dahulu, kemudian klik "Tandai Selesai" di kanan atas.
            </div>
        </div>
    );
  }
